...
class XnorGemm : public OpKernel {
	
	public:
	
	explicit XnorGemm(OpKernelConstruction* context) : OpKernel(context) {}
	...
	void Compute(OpKernelContext* context) override {
		...
		DCHECK_EQ(a_shape.dim_size(1) % (32*16), 0);
...
#define REGISTER_CPU(type)                                          	\
  REGISTER_KERNEL_BUILDER(                                       		\
      Name("XnorGemm").Device(DEVICE_CPU).TypeConstraint<type>("T"), 	\
      XnorGemm<CPUDevice, type>);
REGISTER_CPU(float);
REGISTER_CPU(int32);
...
#define REGISTER_GPU(type)     											\
  extern template struct XnorGemmFunctor<GPUDevice, type>;					\
  REGISTER_KERNEL_BUILDER(                                       		\
      Name("XnorGemm").Device(DEVICE_GPU).TypeConstraint<type>("T"), 	\
      XnorGemm<GPUDevice, type>);
REGISTER_GPU(float);
REGISTER_GPU(int32);
#endif  // GOOGLE_CUDA

...
template <typename T, typename To>
__global__ void concantenateRowsSignsShared_ll(T* mtx, To* sign_mtx)
{
	__shared__ To signs[64];
	
	int wlen = sizeof(To);
	int mult = wlen / 32;
	
	// const int tid = blockIdx.x * blockDim.x + threadIdx.x;
	// signs[threadIdx.x] = (mtx[tid] >= 0);
...
__global__ void matmulCudaKernelGlobalT(Ti* a_mtx, Ti* b_mtx, T* c_mtx, int m, int n, int k)
{
	int trow = blockIdx.y * blockDim.y + threadIdx.y;
	int tcol = blockIdx.x * blockDim.x + threadIdx.x;
...
void XnorMatmulFunctor<GPUDevice, T>::operator()(const GPUDevice& d, T* a_mtx, T* b_mtx, T* out, int m, int n, int k)
{
	...
	// block_count = (m*n)/64;
	// concantenateRowsSignsShared_ll<T> <<<block_count, thread_per_block, 0, d.stream()>>> (a_mtx, ac64);
	...
	concantenateColumnsSigns<T, unsigned int> <<<block_count, thread_per_block, 0, d.stream()>>> (b_mtx, bc, n, k, (n*k)/32);
	...
	matmulCudaKernelGlobalT<T, unsigned int><<<block_dims, thread_dims, 0, d.stream()>>>(ac, bc, out, m, n, k);
